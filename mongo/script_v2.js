...
// 7.gte
db.procedimentos.find({ valorProcedimento: { $gte: 500 } })
...
// 11.max
db.funcionarios.max( { $or: [
    { 'endereco.estado' : "PE" },
    { funcao: "Dentista" }
  ] } ).max( { salario: 100 } ).hint( { salario: 1 } )
...
// 14.sort
db.funcionarios.find( ).sort( { amount: -1 } )
